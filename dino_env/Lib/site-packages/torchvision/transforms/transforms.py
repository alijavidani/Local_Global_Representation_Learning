...
def tuning_function(i , max_limit, step = 16):
    low = np.floor(i/step)*step
    high = np.ceil(i/step)*step
    distance_from_low = i - low
    distance_from_high = high - i
        ...
        for t in self.transforms:
            ...
            if isinstance(img,tuple):
                for i in range(1,len(img)):
                    returned_values.append(img[i])
                img = img[0]
        # print(img)
        # print(returned_values)
        return img , returned_values
...
class RandomHorizontalFlip(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        if torch.rand(1) < self.p:
            flip = 1 #[1]
            return F.hflip(img), flip #, Tensor(flip)
        return img, flip #,Tensor(flip)
...
class RandomResizedCrop(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        _, height, width = F.get_dimensions(img)
        #tune the crop:
        # i = tuning_function(i = i, max_limit=height)
        # j = tuning_function(i = j, max_limit=width)
        # h = tuning_function(i = i + h, max_limit=height) - i
        # w = tuning_function(i = j + w, max_limit=width) - j
        
        crop_coordinates = [i,j,h,w]
        crop_coordinates_tensor = Tensor(crop_coordinates)
        return F.resized_crop(img, i, j, h, w, self.size, self.interpolation, antialias=self.antialias), crop_coordinates #, crop_coordinates_tensor
